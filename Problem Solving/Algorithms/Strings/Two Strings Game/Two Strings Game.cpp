...
struct sfa {
    ...
    void push (int c) {
        int cur = ++nodes, p;
        len[cur] = len[last] + 1;
        for(p = last; p && !next[p][c]; 
            p = lnk[p]) next[p][c] = cur;
        if (!p) 
            lnk[cur] = 1; 
        else {
            int q = next[p][c];
    ...
    void grundy_precalc () {
        ...
        for(i = 1; i <= nodes; i++) {
            ...
            for(j = 0; j < 30; j++) 
                was[j] = 0;
            for(j = 0; j < 26; j++) 
                if (next[k][j]) was[grundy[next[k][j]]] = true;
            for(j = 0; j < 30; j++) 
                if (!was[j]) {
                    grundy[k] = j;
                    break;
    ...
    void substr_precalc () {
        ...
        for(i = 1; i <= nodes; i++) 
            for(j = 0; j < 26; j++) 
                if (next[srt[i].second][j]) 
                    ways[next[srt[i].second][j]] += ways[srt[i].second];
        for(i = 1; i <= nodes; i++) 
            grundy_sum[grundy[i]] += ways[i];
    }
...
int main (int argc, char * const argv[]) {
    ...
    cur = 1;
    while (k > 0) {
        if (k <= sfa2.dp[1] - sfa2.grundy_sum[sfa1.grundy[cur]]) 
            break; 
        else 
            k -= sfa2.dp[1] - sfa2.grundy_sum[sfa1.grundy[cur]];
        for(j = 0; j < 26; j++) 
            if (k > sfa1.dp[sfa1.next[cur][j]]) 
                k -= sfa1.dp[sfa1.next[cur][j]]; 
            else {
                putchar('a' + j);
                cur = sfa1.next[cur][j];
                break;
    ...
    while (k > 0) {
        ...
        for(j = 0; j < 26; j++) 
            if (k > sfa2.dp[sfa2.next[cur][j]]) 
                k -= sfa2.dp[sfa2.next[cur][j]]; 
            else {
                putchar('a' + j);
                cur = sfa2.next[cur][j];
                break;
