...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
    ...
    // general approach (Time complexity = O(n*m))
    /*
    for(int i =0; i < keyboards.size(); i++){
        for(int j = 0; j < drives.size(); j++)
        {
            if(keyboards[i]+drives[j] <= b)
                result = max(result, keyboards[i]+drives[j]);
        }
    }
    */
    
    // This code solves the problem in linear time complexity
    sort(keyboards.begin(), keyboards.end());
    sort(drives.begin(), drives.end());
    
    for(int i =0; i < keyboards.size(); i++){
        int remainingBudget = b - keyboards[i];
        if(remainingBudget <= 0)
            break;
        
        auto it = upper_bound(drives.begin(), drives.end(), remainingBudget);
    
        if(it != drives.begin())
        {
            int ind = distance(drives.begin(), prev(it));
            result = max(result, keyboards[i]+drives[ind]);
        }
    }
